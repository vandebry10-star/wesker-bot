...
plugin -d  <file>`)
    ...
    if (action === 'list') {
      ...
      if (!files.length) return m.sendText('belum ada plugin')
      ...
      return m.sendText(`plugin list (${files.length})\n\n${lines.join('\n')}`)
    }

    /* GET */
    if (action === 'get') {
      const name = normalizeName(args[1])
      if (!name) return m.sendText('nama file diperlukan')
      const target = path.join(PLUGIN_DIR, name)
      if (!fs.existsSync(target)) return m.sendText('plugin tidak ditemukan')
      const data = fs.readFileSync(target)
      if (args.includes('-t')) return m.sendText(data.toString())
      return m.sendDocument(data, name, { mimetype: 'application/javascript' })
    }
    ...
    if (action === 'check') {
      ...
      return m.sendText(formatValidation(result, name))
    }

    /* INSTALL / REPLACE */
    if (action === '-i' || action === '-ir' || action === '-iv') {
      if (!store) return m.sendText('storeMessage tidak tersedia')

      const sourceId = m.quoted?.id || m.id
      const stored   = await store.get(sourceId)
      if (!stored?.raw) return m.sendText('pesan sumber tidak ditemukan')
      ...
      if (!code?.trim()) return m.sendText('pesan bukan kode plugin')

      let name = args[1]
      if (!name) {
        const match = code.match(/command\s*:\s*\[\s*['"`](\w+)/)
        if (!match) return m.sendText('gagal deteksi nama plugin')
        name = match[1]
      }
      ...
      if (fs.existsSync(target) && action === '-i')
        return m.sendText(`plugin "${file}" sudah ada — pakai -ir untuk replace`)

      const strict = action === '-iv'
      const result = await validatePlugin(code, file, pm)

      if (!result.ok)
        return m.sendText(formatValidation(result, file) + '\n\nplugin tidak diinstall')
    ...
    if (action === 'on' || action === 'off') {
      const cmd = args[1]
      if (!cmd) return m.sendText('command diperlukan')
      if (action === 'on') pm.enable(cmd)
      else pm.disable(cmd)
      return m.sendText(`plugin ${action === 'on' ? '🟢 enabled' : '🔴 disabled'}: ${cmd}`)
    }
    ...
    if (action === '-d') {
      ...
      return m.sendText(`🗑️ plugin dihapus: ${name}`)
    }

    return m.sendText('aksi tidak dikenal')
  }
}